...
- **Set pre-season league positions (gameweek 1 of the season only — not every competition)** using the manual override editor at `/admin/standings` — since there are no real results yet at the true start of a season, this is where you'd enter odds-based or predicted starting positions. This only ever happens once per *season*, not once per *competition*: if you're setting up a second competition partway through a season (e.g. a "second half" starting in January), skip this step entirely — sync the real league table instead (`/admin/sync` → Sync Standings), since real results already exist by then. Once you do start syncing real standings, don't go back and manually override again, or the next automatic sync will just overwrite your manual entry anyway.
- **Set the scoring quartiles** at `/admin/quartiles` (splits teams into four bands of difficulty — used to work out how many points a result is worth) and **the draft tiers** at `/admin/draft-tiers` (separate from quartiles — this is what players pick their "double-use" team from). The table doesn't reset between competitions in the same season — a new one just carries on from wherever it currently stands.
